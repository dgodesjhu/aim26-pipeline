...
with st.sidebar:
    ...
    st.divider()
    st.subheader("Evaluator personas")
    st.caption(
        "Each evaluator scores the page from a different perspective. "
        "Minimum scoring means one skeptical evaluator can block any criterion. "
        "Try removing one and see how the score changes — that reveals which "
        "persona is most constraining for your brief."
    )
    use_cro    = st.checkbox("Conversion Rate Optimizer", value=True)
    use_reader = st.checkbox("Target Reader",             value=True)
    use_voice  = st.checkbox("Brand Voice Auditor",       value=True)
    evaluator_indices = [
        i for i, active in enumerate([use_cro, use_reader, use_voice]) if active
    ]
...
if run_button:
    ...
    for field, value in parsed.items():
        if isinstance(value, list):
            preview = "; ".join(value)[:100]
        else:
            preview = str(value)[:100]
        table_rows.append({"Field": field, "Value (first 100 chars)": preview})
    st.table(table_rows)
    ...
    while iteration < max_iterations:
        ...
        for eval_name, eval_text in eval_result.get("evaluations", []):
            with st.expander(f"Evaluator: {eval_name}", expanded=False):
        ...
        # Synthesis
        synthesis = eval_result.get("synthesis", "")
        if synthesis:
    ...
    with tabs[tab_idx]:
        ...
        for i, (er, s) in enumerate(zip(all_evaluations, all_scores)):
            ...
            with st.expander(
                ...
                # Build per-criterion comparison table
                evaluator_score_maps = {}
                for ev_name, ev_text in er.get("evaluations", []):
                    rows = parse_evaluation_rows(ev_text)
                    evaluator_score_maps[ev_name] = {
                        r["Criterion"]: int(r["Score"].split("/")[0]) for r in rows
                    }

                ev_names = [name for name, _ in er.get("evaluations", [])]
                short_labels = [n.split()[0] for n in ev_names]  # Conversion, Target, Brand

                table_rows = []
                for c_num in range(1, n_total_criteria + 1):
                    row: dict = {"Criterion": c_num}
                    mins = []
                    for ev_name, label in zip(ev_names, short_labels):
                        sc = evaluator_score_maps.get(ev_name, {}).get(c_num)
                        row[label] = f"{sc}/3" if sc is not None else "?"
                        if sc is not None:
                            mins.append(sc)
                    row["Min"] = f"{min(mins)}/3" if mins else "?"
